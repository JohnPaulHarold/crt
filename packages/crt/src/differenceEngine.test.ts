...
describe('differenceEngine - advanced diffing', () => {
	test('it updates text content', () => {
		const existing = stringToHTML(`<p>Old text</p>`) as Element;
		const template = stringToHTML(`<p>New text</p>`) as Element;
		diff(template.cloneNode(true), existing);
	...
	test('it replaces a node if the tag name is different', () => {
		...
		const existing = stringToHTML(`<span>Old node</span>`) as Element;
		parent.appendChild(existing);
		const template = stringToHTML(`<p>New node</p>`) as Element;
	...
	test('it preserves focus on an element after a diff', () => {
		...
		const btn2 = parent.querySelector('#btn2') as HTMLButtonElement;
		btn2.focus();
		...
		const newBtn2 = parent.querySelector('#btn2') as HTMLButtonElement;
	...
	test('stringToHTML should convert a valid HTML string to a DOM node', () => {
		...
		const node = stringToHTML(html) as Element;
