...
function diffAttributes(vdom: HTMLElement, dom: HTMLElement) {
	...
	for (let i = 0; i < propsToSync.length; i++) {
		...
		const domProps = dom as unknown as Record<string, unknown>;
		const vdomProps = vdom as unknown as Record<string, unknown>;
		...
		if (vdomProps[prop] !== domProps[prop]) {
			domProps[prop] = vdomProps[prop];
		}
	}
}
...
function _diff(vdom: Node, dom: Node) {
	...
	if (vdom instanceof HTMLElement && dom instanceof HTMLElement) {
...
export function diff(vdom: Node, dom: Node, options: DiffOptions = {}) {
	...
	const preservedAttributeState: Record<string, Record<string, string>> = {};
