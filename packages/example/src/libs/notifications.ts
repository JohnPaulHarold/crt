...
function handleNotification(notifications: HTMLElement[]) {
...
export interface NotificationsServiceInterface {
	outlet: HTMLElement;
	count: number;
	timers: Record<string, number | undefined | NodeJS.Timeout>;
	notificationsQueue: BatchedQueueInstance<HTMLElement>;
	sendNotification: (el: HTMLElement) => void;
	clearNotification: (id: string) => void;
	_resetForTesting?: () => void;
}

export const NotificationsService: NotificationsServiceInterface = {
	outlet: document.createElement('div'),
	count: 0,
	timers: {},
	notificationsQueue: createBatchedQueue(handleNotification, 100, 5),

	sendNotification: function (el: HTMLElement) {
	...
	clearNotification: function (id: string) {
