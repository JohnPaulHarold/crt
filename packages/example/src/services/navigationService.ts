import type { PubSubInstance } from '../state/PubSub.js';
 ...
 */
function createNavigationService(): NavigationServiceInstance {
	let _scope: HTMLElement | undefined = undefined;
	let _lastFocus: HTMLElement | undefined = undefined;
	let _globalKeyDownHandler: ((event: KeyboardEvent) => void) | null = null;
	...
	function clearFocus() {
		...
		collectionToArray<HTMLElement>(els).forEach((el: Element) =>
	 ...
	 */
	function getElementContainer(target: HTMLElement): HTMLElement[] {
		let a = target;
		const els = [];
	 ...
	 * @private
	 */
	function getContainerName(containerEl: HTMLElement): string {
	 ...
	 * @private
	 */
	function getItemName(itemEl: HTMLElement): string {
	...
	const service: NavigationServiceInstance = {
		/**
 ...
 */
function createDummyService(): NavigationServiceInstance {
	const dummyPubSub = {
		on: () => {},
		off: () => {},
		emit: () => {},
...
export const navigationService: NavigationServiceInstance = isBrowser
