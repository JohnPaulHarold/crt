  ...
  getData() {
    VideoTypes.forEach((type) => {
      ...
      Codecs.forEach((codec) => {
        ...
        this.data[type][typeAndCodec][DrmType.WIDEVINE].drm = getDrm(DrmType.WIDEVINE, typeAndCodec)?.then(
          (res) => (this.data[type][typeAndCodec][DrmType.WIDEVINE].drm = res),
        );

        this.data[type][typeAndCodec][DrmType.PLAYREADY].drm = getDrm(DrmType.PLAYREADY, typeAndCodec)?.then(
        ...
        this.data[type][typeAndCodec][DrmType.PLAYREADY_LEGACY].drm = getDrm(
          DrmType.PLAYREADY_LEGACY,
          typeAndCodec,
        )?.then((res) => (this.data[type][typeAndCodec][DrmType.PLAYREADY_LEGACY].drm = res));

        this.data[type][typeAndCodec][DrmType.FAIRPLAY].drm = getDrm(DrmType.FAIRPLAY, typeAndCodec)?.then(
    ...
    if (target && this.data) {
      ...
      Object.keys(this.data).forEach((type) => {
        ...
        Object.keys(this.data[type]).forEach((codec) => {
          target?.appendChild(
            a({ href: '#', className: s.codec }, [Codec({ data: this.data[type][codec], codec, type })]),
          );
        });
      });
    }
